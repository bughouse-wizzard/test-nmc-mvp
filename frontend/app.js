...
let sseConnections = {}; // Store SSE connections by search ID
...
async function startSearch(e) {
    ...
    try {
        ...
        const searchData = {
            ...
            ktru_code: document.getElementById('ktru-input').value || "---",
            okpd2_code: null,
            ...
            characteristics_text: specsText,
            manufacturer: document.getElementById('mfr-input').value || null
        };
        ...
        startSSEConnection(searchId);

    } catch (error) {
        console.error('Error starting search:', error);
        showToast('Ошибка при запуске поиска', 'error');
        resetSidebarUI();
    }
}

function startSSEConnection(searchId) {
    // Close existing connection if any
    if (sseConnections[searchId]) {
        sseConnections[searchId].close();
    }

    // Create new SSE connection
    const eventSource = new EventSource(`/api/search/${searchId}/events`);
    ...
    eventSource.addEventListener('result_added', (event) => {
        try {
            const data = JSON.parse(event.data);
            handleSSEEvent(searchId, data);
        } catch (error) {
            console.error('Error parsing result_added event:', error);
        }
    });
    ...
    eventSource.onerror = (error) => {
        ...
        setTimeout(() => {
            if (activeSidebarSearchId === searchId) {
                console.log(`Reconnecting SSE for search ${searchId}...`);
                startSSEConnection(searchId);
            }
        }, 5000);
    };
    ...
    sseConnections[searchId] = eventSource;
}

function handleSSEEvent(searchId, eventData) {
    console.log(`SSE event for search ${searchId}:`, eventData);
    
    // Update UI based on event type
    switch (eventData.type) {
        case 'progress':
            updateSearchProgress(searchId, eventData);
            break;
        case 'result_added':
            addSearchResult(searchId, eventData);
            break;
        case 'done':
            finishSearch(searchId, eventData);
            break;
        case 'error':
            handleSearchError(searchId, eventData);
            break;
    }
}

function updateSearchProgress(searchId, progressData) {
    // Update progress bar in sidebar if this is the active search
    if (activeSidebarSearchId === searchId) {
        const progressPercent = (progressData.processed_count / progressData.found_total) * 100;
        document.getElementById('proc-bar').style.width = `${Math.min(100, progressPercent)}%`;
        document.getElementById('proc-count').innerText = `${progressData.processed_count}/${progressData.found_total}`;
    }

    // Update history item
    const historyItem = MOCK_HISTORY.find(item => item.id === searchId);
    if (historyItem) {
        historyItem.found = progressData.found_total;
        historyItem.status = progressData.status || "В работе";
        renderHistory();
    }
}

function addSearchResult(searchId, resultData) {
    // Here you would add the result to the results table
    // For now, just log it
    console.log('New contract result:', resultData);
    
    // Update badge to show new results
    document.getElementById('badge-new').classList.remove('hidden');
}

function handleSearchError(searchId, errorData) {
    console.error(`Search error for ${searchId}:`, errorData);
    
    // Update history item
    const historyItem = MOCK_HISTORY.find(item => item.id === searchId);
    if (historyItem) {
        historyItem.status = "Ошибка";
        renderHistory();
    }

    // Show error toast
    showToast(`Ошибка поиска: ${errorData.error_message}`, 'error');
    
    // Reset sidebar UI if this is the active search
    if (activeSidebarSearchId === searchId) {
        resetSidebarUI();
    }
}
...
function finishSearch(id, eventData) {
    // Handle both mock data (limitVal) and real SSE data (eventData)
    if (typeof eventData === 'number') {
        // Old mock behavior
        const limitVal = eventData;
        if(searchIntervals[id]) {
            clearInterval(searchIntervals[id]);
            delete searchIntervals[id];
        }
        ...
        if(item) {
            item.status = "Завершен";
            item.found = eventData.total_found || eventData.total_processed || 0;
            
            // Update NMCK value if available
            if (eventData.nmc_value) {
                // You could store this for display in results view
                console.log(`NMCK value calculated: ${eventData.nmc_value}`);
            }
        }
